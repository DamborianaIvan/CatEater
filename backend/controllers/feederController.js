...
const getFeederConfiguration = async (req, res) => {
  ...
  if (!feederId || feederId.trim() === "") {
    return res.status(400).json({ error: "feederId es requerido" });
  }
  ...
  try {
    ...
    if (!feeder) {
      return res.status(404).json({ error: "Feeder no encontrado" });
    }
...
// Enrollment inicial del dispositivo. Utiliza la API Key global únicamente como bootstrap.
